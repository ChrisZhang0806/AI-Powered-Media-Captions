export type Language = 'zh' | 'en';

export const translations = {
    zh: {
        brand: 'AI 智能视频字幕',
        apiKey: 'OpenAI API 密钥',
        configApiKey: '配置 OpenAI API Key',
        apiKeyTip: 'Key 仅保存在本机浏览器中。媒体内容只会在您开始 AI 处理时发送。',
        getApiKey: '获取 API Key',
        viewUsage: '查看余额/用量',
        deleteKey: '删除 Key',
        confirmDelete: '确认删除',
        confirmReset: '当前字幕或处理进度会被清空。确定继续吗？',
        cancel: '取消',
        verifyAndConfirm: '验证并确认',

        mainTitle: 'AI 音视频字幕工作台',
        mainSubtitle: '快速转写、语义断句、双语翻译，并导出 SRT、VTT 或纯文本。',
        uploadTip: '点击或拖拽上传媒体或字幕文件',
        supportFormat: '支持 MP4, TS, MP3, WAV 及 SRT, VTT 格式',

        processMode: '识别模式',
        originalOnly: '仅原文',
        translationOnly: '仅翻译',
        bilingual: '双语对照',
        contextPrompt: '识别背景提示',
        contextPromptTip: '提高专业词汇准确率',
        contextPromptPlaceholder: '输入专有名词、人名或背景说明...',
        targetLang: '翻译语言',
        transStyle: '翻译风格',
        styleLiteral: '直译',
        styleCreative: '创意',
        styleBalanced: '平衡',
        startProcess: '开始 AI 解析',
        textOnly: '仅输出文本（不含时间轴）',

        processing: '正在 AI 智能解析...',
        engineStarting: 'AI 引擎启动中',
        translating: '正在 AI 智能翻译...',
        capturedSegments: '已捕获 {count} 条字幕段',
        showAfterFinish: '完成后显示字幕',

        subtitlePreview: '字幕预览',
        downloadSrt: 'SRT',
        downloadVtt: 'VTT',
        exportSubtitle: '导出字幕',
        contentType: '内容',
        formatType: '格式',
        noSubtitles: '暂无字幕内容',
        playPosition: '播放位置',
        originalContent: '原文内容',
        manage: '管理',

        bilingualMode: '双语对照',
        onlyTranslation: '仅译文',
        onlyOriginal: '仅原文',
        noTranslationNeeded: '无需翻译',
        translateNow: '立即翻译',
        waitingForTranslation: '等待翻译...',
        proofreading: '智能校对中...',
        proofreadingTerms: '正在校对专业术语...',
        original: '原文',
        translation: '译文',

        errorNoApiKey: '请先配置 OpenAI API Key 才能进行操作',
        errorInvalidSub: '无效的字幕文件，请检查格式是否正确',
        errorProcessFailed: '生成失败',
        errorTranslateFailed: '翻译失败',
        errorServerUnavailable: '处理服务暂时不可用，请稍后重试',
        previewUnavailable: '此格式不支持浏览器预览，但仍可直接生成字幕',

        // Progress Details
        uploading: '上传文件到服务器...',
        extractingAudio: '提取音频中...',
        segmenting: '分割音频中...',
        transcribing: '转录中...',
        done: '处理完成',
        analyzing: 'AI 正在分析音频内容',
        prepUpload: '准备上传到 AI...',
        aiRecognizing: 'AI 正在识别语音...',
        organizing: '转录完成，整理结果...',
        preparingEngine: '启动处理引擎...',
        readyForSegment: '准备音频分割',
        extractingDetails: '正在处理视频文件',
        timeWait: '这可能需要一些时间',
        capturedInfo: '已识别 {count} 条字幕',
        segmentInfo: '正在处理第 {index} 个片段',

        // Languages
        langChinese: '中文',
        langEnglish: '英文',
        langJapanese: '日文',
        langKorean: '韩文',
        langFrench: '法文',
        langGerman: '德文',
        langSpanish: '西班牙文',
        langPortuguese: '葡萄牙语',
        langArabic: '阿拉伯语',
        langRussian: '俄语',
        langItalian: '意大利语',
        langVietnamese: '越南语',
        langThai: '泰语'
    },
    en: {
        brand: 'AI Media Captions',
        apiKey: 'OpenAI API key',
        configApiKey: 'Configure OpenAI API key',
        apiKeyTip: 'Your key is saved locally in this browser. Media is sent only when you start AI processing.',
        getApiKey: 'Get API key',
        viewUsage: 'View usage/billing',
        deleteKey: 'Delete key',
        confirmDelete: 'Confirm delete',
        confirmReset: 'This will clear the current captions or processing progress. Continue?',
        cancel: 'Cancel',
        verifyAndConfirm: 'Verify & Confirm',

        mainTitle: 'AI Audio/Video Transcription & Translation',
        mainSubtitle: 'Fast AI-powered transcription, semantic segmentation, and professional translation.',
        uploadTip: 'Click or drag to upload media or subtitle files',
        supportFormat: 'Supports MP4, TS, MP3, WAV and SRT, VTT',

        processMode: 'Process mode',
        originalOnly: 'Original only',
        translationOnly: 'Translation only',
        bilingual: 'Bilingual',
        contextPrompt: 'Context prompt',
        contextPromptTip: 'Improve accuracy for domain terms',
        contextPromptPlaceholder: 'Enter technical terms, names, or background info...',
        targetLang: 'Target language',
        transStyle: 'Translation style',
        styleLiteral: 'Literal',
        styleCreative: 'Creative',
        styleBalanced: 'Balanced',
        startProcess: 'Start AI processing',
        textOnly: 'Text only (no timestamps)',

        processing: 'AI Processing...',
        engineStarting: 'Starting AI engine...',
        translating: 'AI Translating...',
        capturedSegments: 'Captured {count} segments',
        showAfterFinish: 'Subtitles will show here',

        subtitlePreview: 'Subtitle preview',
        downloadSrt: 'SRT',
        downloadVtt: 'VTT',
        exportSubtitle: 'Export',
        contentType: 'Content',
        formatType: 'Format',
        noSubtitles: 'No subtitles',
        playPosition: 'Play position',
        originalContent: 'Original content',
        manage: 'Manage',

        bilingualMode: 'Bilingual',
        onlyTranslation: 'Translation only',
        onlyOriginal: 'Original only',
        noTranslationNeeded: 'No translation needed',
        translateNow: 'Translate now',
        waitingForTranslation: 'Waiting for translation...',
        proofreading: 'Proofreading...',
        proofreadingTerms: 'Refining terminology...',
        original: 'Original',
        translation: 'Translation',

        errorNoApiKey: 'Please configure OpenAI API Key first',
        errorInvalidSub: 'Invalid subtitle file format',
        errorProcessFailed: 'Processing failed',
        errorTranslateFailed: 'Translation failed',
        errorServerUnavailable: 'The processing service is unavailable. Please try again later.',
        previewUnavailable: 'This format cannot be previewed in the browser, but captions can still be generated.',

        // Progress Details
        uploading: 'Uploading to server...',
        extractingAudio: 'Extracting audio...',
        segmenting: 'Segmenting audio...',
        transcribing: 'Transcribing...',
        done: 'Processing finished',
        analyzing: 'AI is analyzing audio content',
        prepUpload: 'Preparing for AI upload...',
        aiRecognizing: 'AI is recognizing speech...',
        organizing: 'Transcription done, organizing result...',
        preparingEngine: 'Starting processing engine...',
        readyForSegment: 'Ready for audio segmenting',
        extractingDetails: 'Processing video file',
        timeWait: 'This may take some time',
        capturedInfo: 'Recognized {count} segments',
        segmentInfo: 'Processing segment #{index}',

        // Languages
        langChinese: 'Chinese',
        langEnglish: 'English',
        langJapanese: 'Japanese',
        langKorean: 'Korean',
        langFrench: 'French',
        langGerman: 'German',
        langSpanish: 'Spanish',
        langPortuguese: 'Portuguese',
        langArabic: 'Arabic',
        langRussian: 'Russian',
        langItalian: 'Italian',
        langVietnamese: 'Vietnamese',
        langThai: 'Thai'
    }
};

export const getTranslation = (lang: Language) => translations[lang];
